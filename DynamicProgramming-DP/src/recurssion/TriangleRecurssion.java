 ...
 *
 */
public class TriangleRecurssion {
	public static void main(String[] args) {
		...
		TriangleRecurssion t = new TriangleRecurssion();
		System.out.println("Min Path in Traiangle : " + t.minPathInTraiangle(arr,0,0, arr.length));
	}
	
	public int minPathInTraiangle(int[][] arr, int i, int j, int n) {
		if(i == n-1) return arr[n-1][j];
		
		int down = arr[i][j] + minPathInTraiangle(arr, i+1, j, n);
		int diagnol = arr[i][j] + minPathInTraiangle(arr, i+1, j+1, n);
