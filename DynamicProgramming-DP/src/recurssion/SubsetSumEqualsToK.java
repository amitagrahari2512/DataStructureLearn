...
public class SubsetSumEqualsToK {
	...
	public static void main(String[] args) {
		...
		SubsetSumEqualsToK s = new SubsetSumEqualsToK();
		System.out.println("SubsetSumEqualsToK present : " +s.subset(arr, n-1, target) );
	}
	
	public boolean subset(int[] arr, int i, int target) {
		if(target == 0) return true;
		if(i == 0) return arr[0] == target;
		
		boolean notTake = subset(arr, i-1, target);
		boolean take = false;
		if(arr[i] <= target) take = subset(arr, i - 1, target - arr[i]);
