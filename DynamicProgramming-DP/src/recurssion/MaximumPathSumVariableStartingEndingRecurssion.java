...
public class MaximumPathSumVariableStartingEndingRecurssion {
	public static void main(String[] args) {
		...
		MaximumPathSumVariableStartingEndingRecurssion p = new MaximumPathSumVariableStartingEndingRecurssion();
		int max = (int)Math.pow(-10, 9);
		for(int i = 0 ; i < n ; i++) {
			max = Math.max(max, p.maxPathvariableStartEnd(arr, m-1, i));
		}
		System.out.println("MaxPathvariableStartEnd " + max);
		
	}
	
	public int maxPathvariableStartEnd(int[][] arr, int i , int j) {
		if(j< 0 || j >= arr[0].length) return (int)Math.pow(-10, 9);
		if(i == 0) return arr[0][j];
		
		int up = arr[i][j] + maxPathvariableStartEnd(arr, i-1 ,j);
		int leftDiagnol = arr[i][j] + maxPathvariableStartEnd(arr, i-1, j-1);
		int rightDiagnol = arr[i][j] + maxPathvariableStartEnd(arr, i-1, j+1);
		
		return Math.max(up, Math.max(leftDiagnol, rightDiagnol));
	}
}
