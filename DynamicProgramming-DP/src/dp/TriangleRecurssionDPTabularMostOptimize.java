 ...
 *
 */
public class TriangleRecurssionDPTabularMostOptimize {
	public static void main(String[] args) {
		...
		TriangleRecurssionDPTabularMostOptimize t = new TriangleRecurssionDPTabularMostOptimize();
		System.out.println("Min Path in Traiangle : " + t.minPathInTraiangle(arr,n));
	}
	
	public int minPathInTraiangle(int[][] arr, int n) {
		int[] front = new int[n];
		for(int i = 0 ; i < n ; i++) {
			front[i] = arr[n-1][i];
		}
		for(int i = n - 2 ; i >=0 ;i--) {
			...
			for(int j = i ; j >=0 ;j--) {
				...
				int diagnol = arr[i][j] + front[j+1];
				curr[j] = Math.min(down,  diagnol);
			}
			front = curr;
		}
