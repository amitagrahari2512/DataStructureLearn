...
public class MaximumPathSumVariableStartingEndingDP {
	public static void main(String[] args) {
		...
		MaximumPathSumVariableStartingEndingDP p = new MaximumPathSumVariableStartingEndingDP();
		int max = Integer.MIN_VALUE;
		for(int i = 0 ; i < n ; i++) {
			max = Math.max(max, p.maxPathvariableStartEnd(arr, dp, m-1, i));
		}
		System.out.println("MaxPathvariableStartEnd " + max);
		
	}
	
	public int maxPathvariableStartEnd(int[][] arr, int[][] dp, int i , int j) {
		if(j< 0 || j >= arr[0].length) return (int)Math.pow(-10, 9);
		if(i == 0) return arr[0][j];
		if(dp[i][j] != -1) return dp[i][j];
		
		int up = arr[i][j] + maxPathvariableStartEnd(arr, dp, i-1 ,j);
		int leftDiagnol = arr[i][j] + maxPathvariableStartEnd(arr, dp, i-1, j-1);
		int rightDiagnol = arr[i][j] + maxPathvariableStartEnd(arr, dp, i-1, j+1);
		
		return dp[i][j] = Math.max(up, Math.max(leftDiagnol, rightDiagnol));
	}
}
