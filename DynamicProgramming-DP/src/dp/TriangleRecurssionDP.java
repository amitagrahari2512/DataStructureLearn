 ...
 *
 */
public class TriangleRecurssionDP {
	public static void main(String[] args) {
		...
		TriangleRecurssionDP t = new TriangleRecurssionDP();
		System.out.println("Min Path in Traiangle : " + t.minPathInTraiangle(arr,dp, 0,0, n));
	}
	
	public int minPathInTraiangle(int[][] arr, int[][] dp, int i, int j, int n) {
		if(i == n-1) return arr[n-1][j];
		
		if(dp[i][j] != -1) return dp[i][j];
		
		int down = arr[i][j] + minPathInTraiangle(arr, dp, i+1, j, n);
		int diagnol = arr[i][j] + minPathInTraiangle(arr, dp, i+1, j+1, n);
