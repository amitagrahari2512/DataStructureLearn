 ...
 *
 */
public class SubsetSumEqualsToKDp {
	...
	public static void main(String[] args) {
		...
		SubsetSumEqualsToKDp s = new SubsetSumEqualsToKDp();
		System.out.println("SubsetSumEqualsToK present : " +s.subset(arr, dp, n-1, target) );
	}
	
	public boolean subset(int[] arr, int[][] dp, int i, int target) {
		if(target == 0) return true;
		if(i == 0) return arr[0] == target;
		
		if(dp[i][target] != -1) return dp[i][target] == 0 ? false : true;
		boolean notTake = subset(arr, dp, i-1, target);
		boolean take = false;
		if(arr[i] <= target) take = subset(arr, dp, i-1, target - arr[i]);
