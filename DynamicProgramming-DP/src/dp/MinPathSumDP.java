...
public class MinPathSumDP {
	public static void main(String[] args) {
		...
		MinPathSumDP min = new MinPathSumDP();
		System.out.println("Min Path Value : " + min.minPathSum(arr,dp, m-1, n-1));
		
	}
	
	public int minPathSum(int[][] arr, int[][] dp, int i, int j) {
		if(i == 0 && j == 0) return arr[i][j];
		if(i < 0 || j < 0) return (int) Math.pow(10, 9);
		
		if(dp[i][j] != -1) return dp[i][j];
		
		int up = arr[i][j] + minPathSum(arr,dp, i-1, j);
		int left = arr[i][j] + minPathSum(arr,dp, i, j-1);
